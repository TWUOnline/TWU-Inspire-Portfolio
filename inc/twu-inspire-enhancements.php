...
class Inspire_Theme_Plugin_Enhancements {
	...
	static function init() {
		...
		if ( ! $instance ) {
			$instance = new Inspire_Theme_Plugin_Enhancements;
		}
	...
	function __construct() {
		...
		$this->plugins = array(
			array(
				'slug'    => 'twu-portfolio',
				'name'    => 'TWU Portfolio Helper',
				'message' => sprintf(
					esc_html__( 'The %1$s is necessary to use some of this theme&rsquo;s features. ', 'illustratr' ),
					'<strong>' . esc_html__( 'TWU Portfolio Helper plugin', 'illustratr' ) . '</strong>' ),
			),
		);
	...
	function admin_notices() {
		...
		if ( get_user_meta( get_current_user_id(), 'twu_inspire_admin_notice', true ) === 'dismissed' ) {
			return;
		}
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' Please activate %1$s. %2$s Once done, go to Settings -&gt; Permalinks and click Save to make sure portfolio URLs work.', 'illustratr' ),
		...
		printf(
			'<div id="twu-inspire-notice" class="notice notice-warning is-dismissible">%s</div>',
...
add_action( 'admin_head', array( 'Inspire_Theme_Plugin_Enhancements', 'init' ) );

function twu_inspire_enqueue_scripts() {
	// Add the admin JS if the notice has not been dismissed
	if ( is_admin() && get_user_meta( get_current_user_id(), 'twu_inspire_admin_notice', true ) !== 'dismissed' ) {

		// Adds our JS file to the queue that WordPress will load
		wp_enqueue_script( 'twu_inspire_admin_script', get_stylesheet_directory_uri() . '/inc/twu-inspire-enhancements.js', array( 'jquery' ), '20180901', true );

		// Make some data available to our JS file
		wp_localize_script( 'twu_inspire_admin_script', 'twu_inspire_admin', array(
			'twu_inspire_admin_nonce' => wp_create_nonce( 'twu_inspire_admin_nonce' ),
		));
	}
}
add_action( 'admin_enqueue_scripts', 'twu_inspire_enqueue_scripts' );
 ...
 */
function twu_inspire_dismiss_admin_notice() {
	// Verify the security nonce and die if it fails
	if ( ! isset( $_POST['twu_inspire_admin_nonce'] ) || ! wp_verify_nonce( $_POST['twu_inspire_admin_nonce'], 'twu_inspire_admin_nonce' ) ) {
		wp_die( __( 'Your request failed permission check.', 'illustratr' ) );
	}
	// Store the user's dimissal so that the notice doesn't show again
	update_user_meta( get_current_user_id(), 'twu_inspire_admin_notice', 'dismissed' );
...
add_action( 'wp_ajax_twu_inspire_admin_notice', 'twu_inspire_dismiss_admin_notice' );
